...
local far = Data.pokemon.FARCHAMP

for _, pair in ipairs({ { "RAYQUAZA_M", ray }, { "MEWTHREE", mew },
                        { "COALOSSAL", coal }, { "FARCHAMP", far } }) do
...
T.eq(far.dex, 197, "Farchamp is 197")
local dexen = { ray.dex, mew.dex, coal.dex, far.dex }
for i = 1, #dexen do
  for j = i + 1, #dexen do
    T.check(dexen[i] ~= dexen[j], "no two mons in the pack share a dex number")
  end
end
...
T.eq(far.spriteFront, far.spriteBack, "Farchamp reuses its front art for the back too")
T.check(far.spriteFront:sub(-14) == "/farchamp.png", "and for Farchamp")
...
T.eq(far.frontSize, 7, "Farchamp's frontSize is explicit too")
...
T.eq(#far.tmhm, 31, "Farchamp's list is Machamp and Farfetch'd combined, deduped")
...
T.eq(far.ability, nil, "Farchamp has no ability -- it's a plain species")
...
T.eq(far.baseStats.attack, 130, "Farchamp attack is 130")
T.eq(far.baseStats.hp + far.baseStats.attack + far.baseStats.defense
     + far.baseStats.speed + far.baseStats.special, 397, "Farchamp's stats total 397")
T.eq(far.types[1], "FIGHTING", "Farchamp is Fighting")
T.eq(far.types[2], "NORMAL", "and Normal")
T.eq(far.palette, "BROWNMON", "Farchamp reuses the built-in brown palette")
